...
def minigame1_handle_quick_reply(sender_id, quick_reply_payload):
    ...
    cdhh.send(sender_id, Attachment.Image(hinh_hlv))
...
def minigame2_handle_result(message, sender_id):
    ...
    if message in huong_tram:
        ...
        cdhh.send(sender_id, Attachment.Image(game2_hlv_url))
        text = "Chính xác!!!!!!!!"
        buttons = [
...
def fansign_menu(sender_id):
    ...
    cdhh.send(sender_id, question, quick_replies=quick_replies,
              metadata="DEVELOPER_DEFINED_METADATA")
    return
...
def image_fs(sender_id, sizeFont, hlv, first_name, last_name, x_Text, y_Text):
    ...
    cdhh.send(sender_id, text1)

    cdhh.send(sender_id, Attachment.Image(
    ...
    cdhh.send(sender_id, Template.Buttons(text2, buttons))
